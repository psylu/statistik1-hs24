...
Die simpelsten Objekte in R sind Vektoren (eine Reihe Werten, wie z.B. eine Spalte einer Exceltabelle). 
...
Datensätzen (`data.frame` oder `tibble`) können Variablen unterschiedlicher Typen kombinieren.
...
- Name: `sub`, Werte: `sub-01`, `sub-02`, `sub-03`, `sub-04`, `sub-05`
- Alter: `age`, Werte: `43`, `54`, `33`. `29`, `22`
- Korrekte Antworten: `accuracy`, Werte: `89`, `77`, `95`, `98`, `99`
...
__b.__ Was ist die Standardabweichung der Daten?

`r webexercises::mcq(c(answer = "ca. 15", "ca. 50", "ca. 100"))`

__c.__ Was ist die der maximale Wert?

__d.__ Was ist der minimale Wert?
...
Wenn ein Package nicht geladen werden, aber eine Funktion daraus verwendet werden soll, kann die Funktion nach dem Package-Name mit `::` aufgerufen werden: `packagename::funktionsname()`.
...
- Reihenfolgen (z.B. zuerst Variable speichern, erst dann ist sie abrufbar)

das __Debugging__, also das Beheben von Fehlern im Code, gehört zu Programmieren dazu.
Lassen Sie sich nicht von Fehlermeldungen aus der Ruhe bringen.
Schauen Sie zuerst, ob es sich "nur" um eine Warnung (`WARNING`) handelt oder ob es sich tatsächlich um eine Fehlermeldung (`ERROR`) handelt. 
